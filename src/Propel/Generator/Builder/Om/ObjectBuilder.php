...
";
    ...
    protected function addGetPrimaryKeySinglePK(string &$script): void
    {
        ...
        // the accessor of a temporal column returns an object, not the PHP type of the column
        $cptype = $pkeys[0]->isTemporalType()
            ? $this->getDateTimeClass($pkeys[0])
            : $pkeys[0]->getPhpType();
...
";
    ...
    protected function addSetPrimaryKeySinglePK(string &$script): void
    {
        ...
        if ($col->isTemporalType()) {
            // the temporal mutator takes anything it can build a date from and is not type hinted
            $docType = 'string|integer|\DateTimeInterface';
            $ctype = '';
        } else {
            $docType = $col->getPhpType();
            $ctype = $docType . ' ';
        }
...
";
        ...
        foreach ($table->getPrimaryKey() as $column) {
            ...
            // resetting null to null is a no-op
            if (($this->fixPrimaryKeyDefaultValues ?? true) && $defaultValue !== 'null') {
                $script .= "
