...
";
    ...
    public function getColumnDDL(Column $col): string
    {
        ...
        $default = '';
        if ($usesCustomSequenceAutoIncrement) {
            $table = $col->getTable();
            if ($table === null) {
...
";
        ...
        if ($table && isset($changedProperties['autoIncrement'])) {
            $fromUsesIdentityAutoIncrement = $this->usesIdentityAutoIncrement($fromColumn);
            $toUsesIdentityAutoIncrement = $this->usesIdentityAutoIncrement($toColumn);
            $fromUsesCustomSequenceAutoIncrement = $this->usesCustomSequenceAutoIncrement($fromColumn);
            $toUsesCustomSequenceAutoIncrement = $this->usesCustomSequenceAutoIncrement($toColumn);

            if ($toUsesIdentityAutoIncrement && !$fromUsesIdentityAutoIncrement) {
                $ret .= sprintf(
                    $pattern,
                    $this->quoteIdentifier($table->getName()),
                    $colName . ' ADD GENERATED BY DEFAULT AS IDENTITY',
                );
            }

            if (!$toColumn->isAutoIncrement() && $fromUsesIdentityAutoIncrement) {
                $ret .= sprintf(
                    $pattern,
                    $this->quoteIdentifier($table->getName()),
                    $colName . ' DROP IDENTITY IF EXISTS',
                );
            }

            if ($toUsesCustomSequenceAutoIncrement) {
                $seqName = $this->getSequenceName($table);
                $defaultValue = $this->getSequenceDefaultValueExpression($table);
                $toColumn->setDefaultValue(new ColumnDefaultValue($defaultValue, ColumnDefaultValue::TYPE_EXPR));
...
",
            ...
            if (!$toColumn->isAutoIncrement() && $fromUsesCustomSequenceAutoIncrement) {
                $seqName = $this->getSequenceName($fromTable);
                $changedProperties['defaultValueValue'] = [$this->getSequenceDefaultValueExpression($fromTable), null];
